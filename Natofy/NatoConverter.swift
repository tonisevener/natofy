...
class NatoConverter {
    ...
    static func convert(text: String) -> [Translation] {
        return text.uppercased().map { (c) -> Translation in
            let translation = NatoConverter.alphabetArray.first(where: {$0.short.first == c})
            return translation ?? Translation(short: c.description, long: c.description)
        }
    }
}
